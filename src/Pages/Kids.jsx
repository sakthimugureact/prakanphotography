...
function Kids() {
  const [kidss,setKidss] = useState([]);
  useEffect(()=>{
    fetch('/data.json').then(res=>res.json()).then(data=>setKidss(...kidss,data.kids));
  },[])
    ...
    <div className="container mt-5 mx-auto">
      <div className="row d-flex align-items-center">
        ...
        <div className="col-md-6">
          <img src="/images/KIDS/image2.png" className='img-fluid d-block mx-auto p-4' />
        </div>
      </div>
    </div>
    <div className="container mx-auto">
      <div className="row">
        {kidss&&kidss.map((val,index)=>(
          <div className='col-md-5 col-xl-4 mx-auto mt-5  position-relative p-3' key={index}>
            <Link to={`/kidshow/${val.id}`} className="text-decoration-none"><img src={`${val.imgsrc}`} className='img-fluid' style={{filter:"brightness(70%)"}}></img><h4 className='position-absolute text-light fw-bold' style={{bottom:"30px",left:"20px"}}>{val.name}</h4></Link>
          </div>
        ))}
      </div>
    </div>
